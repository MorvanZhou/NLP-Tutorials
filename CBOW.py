...
# inputs
tfx = tf.placeholder(tf.int32, [None, 2*skip_window])
tfy = tf.placeholder(tf.int32, [None, ])

emb_dim = 2
embeddings = tf.Variable(tf.random_uniform((len(vocab), emb_dim), -1., 1.))     # [n_vocab, emb_dim]
span_embedded = tf.nn.embedding_lookup(embeddings, tfx)                         # [n, span, emb_dim]
embedded = tf.reduce_mean(span_embedded, axis=1)                                # [n, emb_dim]
